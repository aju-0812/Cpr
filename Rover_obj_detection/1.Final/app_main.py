...
st.set_page_config(page_title="Rover AI Vision (Final)", layout="wide")

st.title("🚜 Caterpillar-AI Autonomous Rover Vision (Final)")
st.markdown("""
...
def load_model():
    ...
    # Look for models in the 'models' subdirectory
    fast_model_path = os.path.join(base_dir, 'models', 'yolo_fast.pt')
    accurate_model_path = os.path.join(base_dir, 'models', 'yolo_accurate.pt')
    
    if os.path.exists(fast_model_path):
        print(f"Loading Fast Model: {fast_model_path}")
        return YOLO(fast_model_path)
    elif os.path.exists(accurate_model_path):
        print(f"Loading Accurate Model: {accurate_model_path}")
        return YOLO(accurate_model_path)
    else:
        st.warning("No custom models found in 'models/' folder. Downloading default YOLOv8n...")
